...
switch ($action) {
    ...
    case 'process':
        ...
        foreach($cart as $product_id => $item) {
            ...
            $stock = $item['stock'];
            $total_price_email = $total_price_email + ($item['unit_price'] * $item['quantity']);            
        ...
        $to_address = $_SESSION['user']['emailAddress'];
        $to_name = $_SESSION['user']['firstName']. ' '. $_SESSION['user']['lastName'];
        $subject = 'My Guitar Shop Order #'. $order_id;
        $body = "Thanks for shopping with us! \n". 
                
                "You ordered: ". implode(', ', $product). "\n".     

                "Your total is: ". $total_price_email. "\n".
                "Sincerely, \n".
                "My Guitar Shop";
        send_email($to_address, $to_name, $subject, $body);
        clear_cart();
